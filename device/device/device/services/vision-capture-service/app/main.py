...
class NoopMqtt:
    """Minimal drop-in replacement when MQTT is disabled."""

    def connect(self):
        logging.getLogger("mqtt").info("MQTT disabled; skip connect")

    def publish_json(self, topic, payload, retain=False):
        logging.getLogger("mqtt").debug("MQTT disabled; drop publish to %s", topic)

    def set_cmd_handler(self, topic, handler):
        logging.getLogger("mqtt").debug("MQTT disabled; ignore subscription %s", topic)
...
camera = Camera(
    ...
    cfg.RESOLUTION,
    cfg.FPS,
    depth_mm_scale=getattr(cfg, "DEPTH_MM_SCALE", 1.0),
    depth_mm_offset=getattr(cfg, "DEPTH_MM_OFFSET", 0.0),
    capture_roi=getattr(cfg, "CAPTURE_ROI", None),
)
ingest = IngestClient(
    cfg.INGEST_BASE_URL,
    cfg.INGEST_API_PATH,
