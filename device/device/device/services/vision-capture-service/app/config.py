...
def _env_bool_optional(key: str) -> Optional[bool]:
    v = os.getenv(key)
...
def _env_int_tuple(key: str) -> Optional[tuple[int, int, int, int]]:
    raw = os.getenv(key, "")
    if not raw:
        return None
    try:
...
class Config:
    ...
    PRESENCE_LINGER_MS: int = _env_int("PRESENCE_LINGER_MS", 500)
    ...
    WAIT_WEIGHT_DEFAULT: Optional[bool] = _env_bool_optional("WAIT_WEIGHT_DEFAULT")
    SCALE_DEFAULT_UNIT: str = os.getenv("SCALE_DEFAULT_UNIT", "g").lower()

    # Depth calibration
    DEPTH_MM_SCALE: float = _env_float("DEPTH_MM_SCALE", 1.0)
    DEPTH_MM_OFFSET: float = _env_float("DEPTH_MM_OFFSET", 0.0)
    CAPTURE_ROI: Optional[tuple[int, int, int, int]] = _env_int_tuple("CAPTURE_ROI")
    WEIGHT_SESSION_DELTA_G: float = _env_float("WEIGHT_SESSION_DELTA_G", 50.0)
    WEIGHT_SESSION_COOLDOWN_SEC: int = _env_int("WEIGHT_SESSION_COOLDOWN_SEC", 30)
    DEPTH_CLUSTER_HEIGHT_MIN_MM: float = _env_float("DEPTH_CLUSTER_HEIGHT_MIN_MM", 30.0)
    DEPTH_CLUSTER_MIN_AREA_PX: int = _env_int("DEPTH_CLUSTER_MIN_AREA_PX", 500)
    DEPTH_CLUSTER_DILATE_ITER: int = _env_int("DEPTH_CLUSTER_DILATE_ITER", 1)
