...
* รองรับกล้อง:
  ...
  * Intel RealSense D4xx / D435 (color → fallback infrared)
* Trigger หลายแบบ:
...
* Metadata สำหรับงานวิเคราะห์:

  * Depth map (16-bit PNG) + สถิติ (min/max/mean/median/std)
  * รองรับ ROI ครอบเฉพาะพื้นที่แท่น (`CAPTURE_ROI`)
  * แปลงความลึก → มิลลิเมตร พร้อมปรับ scale/offset (`DEPTH_MM_SCALE`, `DEPTH_MM_OFFSET`)
  * วิเคราะห์วัตถุจาก depth เป็น cluster (กว้าง/ยาว/สูง ต่อก้อน) ด้วย threshold ปรับได้ (`DEPTH_CLUSTER_*`)
  * ป้องกันภาพซ้ำโดยตรวจน้ำหนักนิ่งรอบก่อน (`WEIGHT_SESSION_DELTA_G`, `WEIGHT_SESSION_COOLDOWN_SEC`)
* REST API:
...
# ========= Camera =========
# "0" | "1" ... (USB) | "rtsp://..." | "http(s)://..." | "realsense" | "D435/D415/D455/D457" | "rs:ir"
CAMERA_SOURCE=0
RESOLUTION=1280x720
FPS=10
IMG_FORMAT=jpg
# ปรับ depth → มม. และครอปเฉพาะพื้นที่แท่น (ถ้าไม่ต้องการให้เว้นว่าง)
DEPTH_MM_SCALE=1.0
DEPTH_MM_OFFSET=0.0
# ตัวอย่าง ROI: x1,y1,x2,y2 (พิกเซล ภาพต้นฉบับ)
...
SCALE_DEFAULT_UNIT=g          # กรณีเครื่องชั่งไม่ส่งหน่วย
WAIT_WEIGHT_DEFAULT=true      # ถ้าระบบมี scale และอยากรอทุกครั้งให้ true
WEIGHT_SESSION_DELTA_G=50     # น้ำหนักต้องเปลี่ยนเกินค่า (g) ถึงจะถ่าย session ใหม่
WEIGHT_SESSION_COOLDOWN_SEC=30# หรือเว้นช่วงเวลานี้ก่อนยอมถ่ายซ้ำ
DEPTH_CLUSTER_HEIGHT_MIN_MM=30 # ตัดสินว่าพิกเซลสูงกว่าพื้นเท่าไรถึงจะนับเป็นวัตถุ
DEPTH_CLUSTER_MIN_AREA_PX=500  # กรองก้อนเล็กๆ (pixel)
DEPTH_CLUSTER_DILATE_ITER=1    # จำนวนรอบ morphological close (0=ไม่เติมช่องว่าง)
...
เพิ่มเติม (เมื่อเปิด depth + ROI):

```json
{
  "frame": {"width_px": 290, "height_px": 340, "format": "jpg", "source": "d435"},
  "depth": {
    "frame_px": {"width": 290, "height": 340},
    "stats_mm": {"min": 960.5, "max": 1354.5, "mean": 1098.5, "median": 1089.5, "std": 53.4},
    "calibration": {"mm_scale": 1.0, "mm_offset": -7.5},
    "intrinsics": {"width": 290, "height": 340, "fx": 606.1, "fy": 605.0, "ppx": 118.5, "ppy": 130.0},
    "path": "/data/spool/....depth.png",
    "clusters": [
      {
        "id": 1,
        "pixel_area": 77672,
        "bbox_px": {"x": 0, "y": 5, "w": 290, "h": 327},
        "centroid_px": {"x": 155.57, "y": 174.15},
        "centroid_mm": {"z": 1086.0, "height": 74.0},
        "height_mm": {"max": 1160.0, "mean": 84.99},
        "dimensions_mm": {"width": 425.11, "length": 528.61, "height": 1160.0}
      }
    ]
  },
  "depth_width_mm": 484.34,
  "depth_length_mm": 557.20,
  "depth_height_mm": 220.0
}
```
...
* **Presence (motion)**
  ...
  * `PRESENCE_LINGER_MS` (กี่มิลลิวินาทีที่ถือว่ายังมีวัตถุหลังเจอ motion)
...
* **Scale (serial)**
  ...
  * ถ้าต้องการใช้ค่าน้ำหนักเป็นตัวป้องกันภาพซ้ำ ตั้ง `WAIT_WEIGHT_DEFAULT=true` (จะรอน้ำหนักนิ่งทุกครั้ง), `WEIGHT_SESSION_DELTA_G` (น้ำหนักต้องเปลี่ยนอย่างน้อยกี่กรัมถึงจะถ่าย session ใหม่), `WEIGHT_SESSION_COOLDOWN_SEC` (เวลาขั้นต่ำก่อนยอมถ่ายซ้ำแม้น้ำหนักใกล้เคียงเดิม)
  * ถ้ารุ่นเครื่องชั่งส่งรูปแบบข้อความต่างไป ให้ปรับ parser ในโค้ด (เมธอด `_parse_weight`)

* **Depth / ROI**

  * `CAPTURE_ROI` ครอปภาพและ depth ให้อยู่บนแท่นชั่ง ลดพื้นที่ไม่เกี่ยวข้อง
  * `DEPTH_MM_SCALE` / `DEPTH_MM_OFFSET` ใช้ปรับเทียบความลึกเป็นมิลลิเมตร (ตั้ง offset จากการวัดระยะกล้อง → พื้นจริง)
  * `DEPTH_CLUSTER_HEIGHT_MIN_MM` เกณฑ์ความสูงจากพื้นที่นับเป็นวัตถุ (ค่าสูงขึ้น → กรอง noise)
  * `DEPTH_CLUSTER_MIN_AREA_PX` กรองก้อนเล็กๆ ที่ไม่ต้องการ
  * `DEPTH_CLUSTER_DILATE_ITER` จำนวนรอบ morphological close เพื่อถมช่องว่าง (ตั้ง 0 ถ้าอยากให้ก้อนแยกจากกันง่าย)
