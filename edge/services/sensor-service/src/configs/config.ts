...
/** ---------- MQTT ---------- */
export const MQTT_BROKER_URL = requireEnv('MQTT_BROKER_URL');

// parse จาก URL เพื่อดึง host/port/proto/cred
let urlUser = '', urlPass = '', urlHost = '', urlPort = 1883, urlProtocol: 'mqtt' | 'mqtts' = 'mqtt';
try {
  ...
  throw new Error(`Invalid MQTT_BROKER_URL: ${MQTT_BROKER_URL}`);
}
