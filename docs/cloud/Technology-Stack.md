...
## ภาพรวมเทคโนโลยี (Technology Overview)

FarmIQ Cloud Layer ใช้สแต็กแบบคลาวด์–เนทีฟบนฐาน **microservices** ที่ขยายได้ (scalable) ดูแลง่าย (maintainable) และเน้น **event‑driven** เป็นแกนของระบบ เพื่อความยืดหยุ่นและรองรับการเติบโตของงานเชิงวิเคราะห์และสตรีมข้อมูลแบบเรียลไทม์
...
- **Python**: 3.11+ (บริการ Analytics และงานประมวลผล)
- **Yarn**: 1.22+ (ตัวจัดการแพ็กเกจ)

### Web Frameworks

#### Node.js Services
- **Express.js**: 4.19.2+ (Web framework ทั่วไป)
- **Fastify**: 4.24.3+ (Framework ประสิทธิภาพสูง ใช้กับ Sensor Streamer)
...
- **Prisma**: 5.7.0+ (Modern DB toolkit ใช้ใน Sensor Streamer)
...
- **swagger-jsdoc**: 6.2.8+ (JSDoc → Swagger)
- **@asteasolutions/zod-to-openapi**: 6.2.0+ (Zod → OpenAPI)
...
### Node.js Services
- **Memory**: 512MB – 2GB ต่อบริการ
- **CPU**: 1–2 คอร์ ต่อบริการ
- **Connection Pool**: สูงสุด 20 connections
- **Timeout**: 30 วินาที

### Python Services
- **Memory**: 1GB – 4GB ต่อบริการ
- **CPU**: 2–4 คอร์ ต่อบริการ
...
- **Rate Limiting**: ต่อ IP และต่อผู้ใช้

### Data Protection
- **TLS/SSL**: เข้ารหัสทุกการสื่อสาร
- **Database Encryption**: เข้ารหัสข้อมูลขณะพัก (at rest)
- **API Keys**: จัดเก็บและ rotate อย่างปลอดภัย
- **Input Validation**: ใช้ Zod schemas
- **SQL Injection**: ป้องกันผ่าน ORM

### Monitoring & Logging
- **Structured Logging**: Pino (Node.js), Python logging
- **Metrics**: Prometheus + Grafana
- **Audit Logs**: บันทึกการทำงานที่สำคัญ
- **Error Tracking**: ศูนย์กลางจัดการข้อผิดพลาด

---

*เวอร์ชันเอกสาร: 2024-01-15*
